...
The [course statement and GenAI policy](../GenAI使用規範.md) apply throughout the semester. Students should review them after reading the [complete syllabus](week1_main.md). Detailed instructions are provided for [Report 1](../10/week10_main.md), [Report 2](../14/week14_main.md), and the [final report](../16/week16_main.md).

## How assessment evidence is used

Grades are based only on work submitted through the course LMS or directly observed in class. Team scores apply to the shared submission. Individual presentation, oral response, review, and comparison scores are based on each student's own evidence. Rankings received from classmates do not determine a team's grade.
...
Class participation is evaluated across the 12 teaching weeks: Weeks 2–8, 11–13, 15, and 18. Report weeks and the Week 9 travel break are evaluated under their own rules and do not create an additional participation requirement.

For each teaching week, students complete the evidence required in the main file and submit either the requested file or an LMS entry that identifies the file, section, version, and result to be reviewed. The normal deadline is 48 hours after the scheduled end of class. A mutable cloud link does not replace a submitted snapshot. Preparation for each teaching week is expected to require approximately 45–75 minutes in addition to class time.

The instructor evaluates the complete collection of weekly work rather than treating attendance or one correct answer as sufficient evidence. Brief progress feedback is provided in class. Cumulative rubric status is reported after Weeks 5, 8, and 13, with the final participation evaluation after Week 18. A correction should preserve the earlier attempt and show what changed.

| Area | Points | Observable evidence |
| --- | ---: | --- |
| In-class implementation and problem-solving | 8 | Calculations, code, or portfolio decisions are technically correct and financially interpretable. |
| In-class implementation and problem-solving | 6 | The work records inputs, dates, assumptions, intermediate checks, and results so that another person can follow it. |
| In-class implementation and problem-solving | 6 | The student identifies an error or limitation, tests a plausible cause, and makes an evidence-based correction or revision. |
| Weekly preparation and traceable learning evidence | 4 | Required reading, data, code, or prior-week work is available when the activity begins. |
| Weekly preparation and traceable learning evidence | 4 | Submitted notes or artifacts identify the relevant source, version, assumptions, and result. |
| Weekly preparation and traceable learning evidence | 2 | Evidence is submitted by the stated deadline and is organized well enough to review. |
| Discussion, oral responses, and evidence-based peer feedback | 4 | Contributions and questions refer to a financial assumption, calculation, implementation choice, or observed result. |
| Discussion, oral responses, and evidence-based peer feedback | 4 | Oral responses distinguish what the evidence shows from what remains uncertain and identify a reasonable next check. |
| Discussion, oral responses, and evidence-based peer feedback | 2 | Feedback is specific, respectful, and uses the same comparison condition for the approaches being discussed. |

The three rows for in-class work total 20 points, the three preparation rows total 10 points, and the three discussion rows total 10 points.

## Submission formats shared by all reports

The course LMS is the official submission channel and its timestamp identifies the submitted version. Unless a report file states otherwise, submit:

- a searchable PDF or Markdown report;
- executable `.ipynb` or `.py` files and any required local modules;
- a plain-text, YAML, or equivalent environment record with software and package versions;
- a data record that identifies the source, retrieval date, fields, period, transformations, and distribution restrictions;
- result files needed to verify reported tables or figures;
- a PDF copy of the presentation;
- a contribution statement that identifies each member's work by task and file or section; and
- the disclosure required by the GenAI policy.

Do not submit credentials or place restricted data in a public repository. When raw data cannot be redistributed, submit authorized derived evidence together with retrieval and reconstruction instructions. A cloud notebook may be included for convenience, but the submitted snapshot is the evaluated version.
...
For comparison activities, the frozen presentation and required common result table are available only to the enrolled class through the LMS. Team code, restricted raw data, individual rankings, and grading records are not made public. Individual rankings are visible to the instructor; only an anonymous class summary is returned to students.
